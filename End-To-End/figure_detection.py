...
from text_detection import get_text_bounding_boxes
...
def area_of_overlapping_rectangles(a, b):
    ...
    """
    dx = max(a[0], b[0]) - min(a[2], b[2])  # xmax, xmax, xmin, xmin
...
def detect_figures(
    ...
    text_area_overlap_threshold=0.10,
    max_area_percentage=0.70,
    do_color_check=True,
    do_text_check=True,
):
    ...
    Args:
        ...
        max_area_percentage (float, optional): The maximum percentage of the area of the 
            original image that a figure can take up. If the figure uses more are than 
            ``original_image_area*max_area_percentage`` then the figure will be discarded. 
            Defaults to 0.70.
    ...
    if do_text_check:
        text_bounding_boxes = get_text_bounding_boxes(image, east_path)
    ...
    max_area = int(max_area_percentage * image_area)
    ...
    for box in bounding_boxes:
        ...
        if (image_height // 3 * image_width // 6) < area < max_area:
            ...
            if do_color_check:
                roi_is_color = (
                    detect_color_image(potential_figure, MSE_cutoff=200) == "color"
                )
...
def all_in_folder(path, remove_original=False, **kwargs):
    ...
    for item in tqdm(images, total=len(images), desc="> Figure Detection: Progress"):
        ...
        if os.path.isfile(current_path) and OUTPUT_PATH_MODIFIER not in str(
            ...
            _, output_paths = detect_figures(current_path, **kwargs)
