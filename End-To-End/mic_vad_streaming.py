...
def main(ARGS):
    # Change input rate if --rate_auto
    if ARGS.rate_auto:
        pa = pyaudio.PyAudio()
        device_info = pa.get_device_info_by_index(ARGS.device)
        ARGS.rate = int(device_info['defaultSampleRate'])
    ...
    vad_audio = VADAudio(aggressiveness=ARGS.vad_aggressiveness,
                         ...
                         file=ARGS.file)
    print("> MIC VAD Streaming: Listening (ctrl-C to stop)...")
    ...
    try:
        for frame in frames:
            if frame is not None:
                ...
                print("> MIC VAD Streaming: Recognized: %s" % text)
                if ARGS.write_transcript:
                    transcript += (text + " ")
                stream_context = model.createStream()
    except KeyboardInterrupt:
        print("> MIC VAD Streaming: Ending VAD.")
        pass
    if ARGS.write_transcript:
        print("> MIC VAD Streaming: Writing final transcript to file " + ARGS.write_transcript)
        with open(ARGS.write_transcript, "a+") as file:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--write_transcript', type=str,
                        help="Optional path to save the final concatenated output of all recognized text pieces.")
    ...
    parser.add_argument('-ar', '--rate_auto', action='store_true',
                        help="Automatically set the --rate (input device sampling rate) to its default rate according to pyaudio.")
