...
class LectureSummarizer():
    def __init__(self, params):
        ...
        self.final_data = {"structured_summary": None, "lecture_summary": None, "transcript": None}
    ...
    def step_extract_frames(self):
        output_path = getattr(self.params, "extract_frames_output_path", self.root_process_folder / "frames")
        extract_frames(self.params.video_path, self.params.extract_frames_quality, output_path, self.params.extract_every_x_seconds)

    @time_this
    def step_classify_slides(self):
        frames_dir = getattr(self.params, "frames_dir", self.root_process_folder / "frames")
        frames_sorted_dir, _, _ = classify_frames(frames_dir, model_path=self.params.slide_classifier_model_path)
        self.frames_sorted_dir = frames_sorted_dir

    @time_this
    def step_black_border_removal(self):
        self.frames_sorted_dir = getattr(self, "frames_sorted_dir", self.root_process_folder / "frames_sorted")
        
        slides_dir = getattr(self.params, "slides_dir", self.frames_sorted_dir / "slide")
        slides_noborder_dir = getattr(self.params, "slides_noborder_dir", self.frames_sorted_dir / "slides_noborder")
    ...
    def step_perspective_crop(self):
        self.frames_sorted_dir = getattr(self, "frames_sorted_dir", self.root_process_folder / "frames_sorted")
        self.slides_noborder_dir = getattr(self, "slides_noborder_dir", self.frames_sorted_dir / "slides_noborder")
        
        presenter_slide_dir = getattr(self.params, "presenter_slide_dir", self.frames_sorted_dir / "presenter_slide")
        imgs_to_cluster_dir = getattr(self.params, "imgs_to_cluster_dir", self.frames_sorted_dir / "imgs_to_cluster")
        ...
        if os.path.exists(presenter_slide_dir):
            ...
            ) = sift_matcher.match_features(self.slides_noborder_dir, presenter_slide_dir)
    ...
    def step_cluster_slides(self):
        self.frames_sorted_dir = getattr(self, "frames_sorted_dir", self.root_process_folder / "frames_sorted")
        self.imgs_to_cluster_dir = getattr(self, "imgs_to_cluster", self.frames_sorted_dir / "imgs_to_cluster")
        self.slides_noborder_dir = getattr(self, "slides_noborder_dir", self.frames_sorted_dir / "slides_noborder")
        ...
        if self.params.cluster_method == "normal":
            ...
            segment_cluster = SegmentCluster(self.imgs_to_cluster_dir, model_path=self.params.slide_classifier_model_path)
    ...
    def step_slide_structure_analysis(self):
        self.frames_sorted_dir = getattr(self, "frames_sorted_dir", self.root_process_folder / "frames_sorted")
        self.cluster_dir = getattr(self, "cluster_dir", self.frames_sorted_dir / "slide_clusters")
        self.best_samples_dir = getattr(self, "best_samples_dir", self.cluster_dir / "best_samples")

        ocr_raw_output_file = getattr(self.params, "ocr_raw_output_file", self.root_process_folder / "slide-ocr.txt")
        ocr_json_output_file = getattr(self.params, "ocr_json_output_file", self.root_process_folder / "slide-ssa.json")
    ...
    def step_extract_figures(self):
        ...
        self.cluster_dir = getattr(self, "cluster_dir", self.frames_sorted_dir / "slide_clusters")
        self.best_samples_dir = getattr(self, "best_samples_dir", self.cluster_dir / "best_samples")
        self.ocr_json_output_file = getattr(self, "ocr_json_output_file", self.root_process_folder / "slide-ssa.json")

        figures_dir = getattr(self.params, "figures_dir", self.cluster_dir / "best_samples_figures")
        os.makedirs(figures_dir, exist_ok=True)

        figure_paths = figure_detection.all_in_folder(self.best_samples_dir, east=self.params.east_path)
    ...
    def step_transcribe_audio(self):
        ...
        audio_path = getattr(self.params, "audio_path", self.root_process_folder / "audio.wav")
        transcript_output_file = getattr(self.params, "transcript_output_file", self.root_process_folder / "audio.txt")
        transcript_json_output_file = getattr(self.params, "transcript_json_output_file", self.root_process_folder / "audio.json")
            ...
            try:
                if self.params.chunk == "silence":
                    ...
                    stt_model = transcribe.load_model(
                        self.params.transcription_method, self.params.transcribe_model_dir
                    )
                    ...
                    transcript, transcript_json = transcribe.process_segments(
                        segments, stt_model, method=self.params.transcription_method, audio_length=audio_length, do_segment_sentences=self.params.transcribe_segment_sentences
                    )

                else:  # if not chunking
                    transcript, transcript_json = transcribe.transcribe_audio(
                        audio_path, method=self.params.transcription_method, model=self.params.transcribe_model_dir
                    )
    ...
    def step_summarize(self):
        self.ocr_raw_output_file = getattr(self, "ocr_raw_output_file", self.root_process_folder / "slide-ocr.txt")
        with open(self.ocr_raw_output_file, "r") as ocr_file:
            ocr_results_flat = ocr_file.read()
        self.ocr_json_output_file = getattr(self, "ocr_json_output_file", self.root_process_folder / "slide-ssa.json")

        self.transcript_output_file = getattr(self, "transcript_output_file", self.root_process_folder / "audio.txt")
        ...
        self.transcript_json_output_file = getattr(self, "transcript_json_output_file", self.root_process_folder / "audio.json")
        ...
        if self.params.summarization_mods != "none" and self.params.summarization_mods is not None:
        ...
        if (
            self.params.summarization_ext != "none" and self.params.summarization_ext is not None
        ...
        if (
            self.params.summarization_abs != "none" and self.params.summarization_abs is not None
        ):  # if abstractive method was specified
            lecture_summarized = generic_abstractive(
                summarized_ext, self.params.summarization_abs,
            )
