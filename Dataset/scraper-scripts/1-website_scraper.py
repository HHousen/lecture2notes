...
if csv_path.is_file():
    ...
    df = pd.DataFrame(columns=["date","provider","video_id","download_link","title","description","thumbnail_default","thumbnail_medium","thumbnail_high","downloaded"])
...
df.loc[len(df.index)]=[date,"website",video_id,page_link,video_download_link,page_title,description,0,0,0,"false"]
df.to_csv(csv_path)